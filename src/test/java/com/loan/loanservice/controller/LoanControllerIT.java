...
public class LoanControllerIT {
    ...
    private MockMvc mvc;
    @Autowired
    private LoanPlanGeneratorService generatorService;
    @Autowired
    ...
    public void shouldReturnSuccessfullResponseWithLoanPlan() throws Exception {
        ...
        String valueAsString = objectMapper.writer().withDefaultPrettyPrinter().writeValueAsString(request);
        ...
        mvc.perform(MockMvcRequestBuilders.post(URI)
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].principal",
                Matchers.is(5000.0)))
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].initialOutstandingPrincipal",
                Matchers.is(5000)))
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].borrowerPaymentAmount",
                Matchers.is(5020.83)))
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].interest",
                Matchers.is(20.83)))
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].remainingOutstandingPrincipal",
                Matchers.is(0.0)))
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].date",
                Matchers.is("2020-05-01T00:00:01")));
    }
  ...
  public void shouldThrowValidationExceptionWhenAmountIsZero() throws Exception {
    ...
    public void shouldThrowValidationExceptionWhenNominalRateIsZero() throws Exception {
        ...
        Assertions.assertThat(mvcResult.getResolvedException())
            .isInstanceOf(MethodArgumentNotValidException.class);
        Assertions.assertThat(mvcResult.getResolvedException().getMessage())
            .contains("[nominalRate],1]; default message [nominalRate(annual) must be greater than or equal to 1]");
    }
